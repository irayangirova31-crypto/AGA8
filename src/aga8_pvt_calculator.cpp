...
// ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ: возведение в степень с проверкой (для целых степеней)

static double powInt(double base, int exp) {
    if (exp == 0) return 1.0;
...
static double calculateBijStar(int n, int i, int j) {
    ...
    // Примечание: если коэффициент = 0, то член = 1 (так как a^0 = 1)
